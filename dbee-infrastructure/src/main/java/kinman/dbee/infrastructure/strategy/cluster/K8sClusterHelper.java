...
import kinman.dbee.infrastructure.strategy.cluster.k8s.K8sDeploymentHelper;
...
public class K8sClusterHelper {
	...
	public static void openLogCollector(ApiClient apiClient) {
		...
		if (!fileBeatFile.exists()) {
			LogUtils.throwException(logger, MessageCodeEnum.FILE_BEAT_K8S_FILE_NONEXISTENCE);
		}
	...
	public static V1ConfigMap dbeeConfigMap() {
	 ...
	 */
	public static void createDbeeConfig(ClusterPO clusterPO, CoreV1Api coreApi) {
		try {
			...
			for(V1Namespace n : namespaceList.getItems()) {
				...
				V1ConfigMapList list = coreApi.listNamespacedConfigMap(namespace, null, null, null, null,
						K8sUtils.getSelectorKey(K8sUtils.DBEE_CONFIGMAP_NAME), null, null, null, null, null);
				
				V1ConfigMap configMap = K8sClusterHelper.dbeeConfigMap();
				if(CollectionUtils.isEmpty(list.getItems())) {
					...
					coreApi.replaceNamespacedConfigMap(K8sUtils.DBEE_CONFIGMAP_NAME,
							namespace, configMap, null, null, null, null);
				}
			}
		} catch (ApiException e) {
			String message = e.getResponseBody() == null ? e.getMessage() : e.getResponseBody();
			LogUtils.throwException(logger, message, MessageCodeEnum.DBEE_SERVER_URL_FAILURE);
		}
	}
	 ...
	 */
	public static void deleteDbeeConfig(ClusterPO clusterPO, CoreV1Api coreApi) {
		V1ConfigMap configMap = new V1ConfigMap();
		configMap.setApiVersion("v1");
		configMap.setKind("ConfigMap");
		V1ObjectMeta meta = new V1ObjectMeta();
		meta.setName(K8sUtils.DBEE_CONFIGMAP_NAME);
		meta.setLabels(K8sClusterHelper.dbeeLabel(K8sUtils.DBEE_CONFIGMAP_NAME));
		configMap.setMetadata(meta);
		...
		try {
			...
			String selector = K8sUtils.getSelectorKey(K8sUtils.DBEE_CONFIGMAP_NAME);
			for(V1Namespace n : namespaceList.getItems()) {
				String namespace = n.getMetadata().getName();
				if(!K8sUtils.DBEE_NAMESPACE.equals(namespace)
						&& K8sUtils.getSystemNamspaces().contains(namespace)) {
	...
	public static Map<String, String> deploymentLabel(DeploymentContext context) {
		Map<String, String> labels = K8sClusterHelper.dbeeLabel(context.getDeploymentName());
