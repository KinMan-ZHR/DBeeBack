...
import kinman.dbee.infrastructure.strategy.cluster.model.DockerConfigJson.Auth;
...
public class K8sClusterStrategy implements ClusterStrategy {
	...
	public Void createSecret(ClusterPO clusterPO, ImageRepo imageRepo) {
		try(KubernetesClient client = client(clusterPO.getClusterUrl(), clusterPO.getAuthToken())){
			...
			meta.setLabels(K8sUtils.dbeeLabel(K8sUtils.DOCKER_REGISTRY_KEY));
	...
	private ObjectMeta ingressMeta(String appName, DeploymentContext context) {
		...
		metadata.setLabels(K8sUtils.dbeeLabel(appName));
	...
	private ObjectMeta deploymentMetaData(String deploymentName) {
		...
		metadata.setLabels(K8sUtils.dbeeLabel(deploymentName));
		return metadata;
	}
	...
	public Void createDbeeConfig(ClusterPO clusterPO) {
		try(KubernetesClient client = client(clusterPO.getClusterUrl(), clusterPO.getAuthToken())){
			DBeeConfigHelper.writeServerIp(clusterPO, client);
		}
		return null;
	}
	...
	public Void deleteDbeeConfig(ClusterPO clusterPO) {
		try(KubernetesClient client = client(clusterPO.getClusterUrl(), clusterPO.getAuthToken())){
			DBeeConfigHelper.deleteServerIp(clusterPO, client);
		}
		return null;
	}
