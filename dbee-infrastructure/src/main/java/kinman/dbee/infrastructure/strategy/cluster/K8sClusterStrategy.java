...
import kinman.dbee.api.enums.ReplicaStatusEnum;
...
import kinman.dbee.infrastructure.strategy.cluster.model.DockerConfigJson;
import kinman.dbee.infrastructure.strategy.cluster.model.Replica;
import kinman.dbee.infrastructure.model.JsonPatch;
...
/**
 * k8s操作功能，该类已过时，请使用{@link kinman.dbee.infrastructure.strategy.cluster.k8s.K8sClusterStrategy}代替。
 * @author Dahai
 */
@Deprecated
public class K8sClusterStrategy implements ClusterStrategy {
	...
	public Void createSecret(ClusterPO clusterPO, ImageRepo imageRepo) {
		...
		meta.setLabels(K8sClusterHelper.dbeeLabel(K8sUtils.DOCKER_REGISTRY_KEY));
	...
	private Map<String, byte[]> dockerConfigData(ImageRepo imageRepo) {
		...
		DockerConfigJson.Auth auths = new DockerConfigJson.Auth();
	...
	private boolean createService(DeploymentContext context) throws ApiException {
		...
		if (CollectionUtils.isEmpty(serviceList.getItems())) {
			...
			path.setOp("replace");
			path.setPath("/spec/selector/dbee-app");
	...
	private V1ObjectMeta ingressMeta(String appName, DeploymentContext context) {
		...
		metadata.setLabels(K8sClusterHelper.dbeeLabel(appName));
	...
	private V1ServiceSpec serviceSpec(DeploymentContext context) {
		...
		spec.setSelector(Collections.singletonMap(K8sUtils.DBEE_LABEL_KEY, context.getDeploymentName()));
	...
	private V1ObjectMeta deploymentMetaData(String deploymentName) {
		...
		metadata.setLabels(K8sClusterHelper.dbeeLabel(deploymentName));
		return metadata;
	}
	...
	private V1LabelSelector specSelector(String deploymentName) {
		...
		selector.setMatchLabels(Collections.singletonMap(K8sUtils.DBEE_LABEL_KEY, deploymentName));
		return selector;
	}
	...
	private List<String> jvmArgsOfDbee(DeploymentContext context) {
	...
	private void initContainerOfDbeeAgent(DeploymentContext context, List<V1Container> containers) {
		if(!TechTypeEnum.SPRING_BOOT.getCode().equals(context.getApp().getTechType())) {
			return;
		}
	...
	private List<V1VolumeMount> volumeMounts(DeploymentContext context) {
		...
		V1VolumeMount dbeeAgent = new V1VolumeMount();
	...
	private List<V1Volume> volumes(DeploymentContext context) {
		...
		V1Volume dbeeAgent = new V1Volume();
		dbeeAgent.setName(K8sUtils.AGENT_VOLUME);
	...
	public List<ReplicaMetrics> replicaMetrics(ClusterPO clusterPO, String namespace) {
		...
		for(PodMetrics metric : metrics) {
			...
			String appLabel = metric.getMetadata().getLabels().get(K8sUtils.DBEE_LABEL_KEY);
	 ...
	 */
	@Override
	public Void createDbeeConfig(ClusterPO clusterPO) {
		ApiClient apiClient = this.apiClient(clusterPO.getClusterUrl(), clusterPO.getAuthToken());
		CoreV1Api coreApi = new CoreV1Api(apiClient);
		K8sClusterHelper.createDbeeConfig(clusterPO, coreApi);
		return null;
	}
	 ...
	 */
	@Override
	public Void deleteDbeeConfig(ClusterPO clusterPO) {
		ApiClient apiClient = this.apiClient(clusterPO.getClusterUrl(), clusterPO.getAuthToken());
		CoreV1Api coreApi = new CoreV1Api(apiClient);
		K8sClusterHelper.deleteDbeeConfig(clusterPO, coreApi);
		return null;
	}
