...
import kinman.dbee.infrastructure.exception.SysException;
...
public class InitializingDBComponent implements InitializingBean {
	...
	private void checkDataPath() throws Exception {
		...
		if (!dataPath.exists()) {
			logger.info("Dbee data path does not exist and create it, path={}",
					componentConstants.getDataPath());
			if (dataPath.mkdirs()) {
				logger.info("Create dbee data path successfully");
			} else {
				throw new SysException("Failed to create dbee data path");
			}
		} else if (!dataPath.isDirectory()) {
			throw new SysException("Dbee data path does not exist");
		} else {
			logger.info("The path of dbee exists and does not need to be created, path={}",
					componentConstants.getDataPath());
		}

		File versionFilePath = Paths.get(dataPath.getAbsolutePath() + File.separator + "version").toFile();
		if (!versionFilePath.exists()) {
			logger.info("Dbee version file does not exist and create it");
			if (versionFilePath.createNewFile()) {
				logger.info("Create dbee version file successfully");
			} else {
				throw new SysException("Failed to create dbee version");
			}
		} else if (!versionFilePath.isFile()) {
			throw new SysException("Dbee version file does not exist");
		} else {
			logger.info("The version file of dbee exists and does not need to be created");
		}
	}
