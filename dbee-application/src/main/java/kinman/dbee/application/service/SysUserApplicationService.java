...
import kinman.dbee.infrastructure.utils.GuavaCacheUtils;
...
public class SysUserApplicationService extends BaseApplicationService<SysUser, SysUserPO> {
	...
	public LoginUser login(UserLoginParam userLoginParam) {
		...
		if(RegisteredSourceEnum.DBEE.getCode().equals(userLoginParam.getLoginSource())
	...
	public LoginUser queryLoginUserByToken(String loginToken) {
		if (StringUtils.isBlank(loginToken)) {
			LogUtils.throwException(logger, MessageCodeEnum.SYS_USER_NOT_LOGIN);
		}
	...
	public List<SysUser> search(LoginUser loginUser, UserSearchParam usersearchParam) {
		//目前只支持从dbee搜索用户，从别的渠道首先登录一下，会保存到dbee系统里
		//如果后期需要从别的渠道搜索用户的话，实现方案如下：
		//记录用户最后的登录渠道，然后从该渠道搜索用户
		UserStrategy userStrategy = userStrategy(RegisteredSourceEnum.DBEE.getCode());
	...
	public Void createUser(UserCreationParam userCreationParam) {
		...
		sysUserParam.setRegisteredSource(RegisteredSourceEnum.DBEE.getCode());
	...
	public Void updatePassword(LoginUser loginUser, PasswordUpdateParam passwordUpdateParam) {
		...
		if (sysUserPO == null) {
			LogUtils.throwException(logger, MessageCodeEnum.SYS_USER_IS_NONEXISTENCE);
		}
	...
	/**
	 * 
	 * 修改角色类型，角色枚举：org.dbee.api.enums.RoleTypeEnum
	 * 
	 * @param
	 * @return
	 */
